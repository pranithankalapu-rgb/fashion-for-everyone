...
export const OnboardingModal: React.FC<OnboardingModalProps> = ({
  ...
  const handleSimulateScan = () => {
    ...
    setTimeout(() => {
      const results: {
        ...
        { skinTone: 'Warm Golden', undertone: 'Warm', hairColor: 'Honey Chestnut', bodyShape: 'Hourglass' },
        { skinTone: 'Cool Rose', undertone: 'Cool', hairColor: 'Espresso Black', bodyShape: 'Rectangle' },
        { skinTone: 'Deep Rich', undertone: 'Warm', hairColor: 'Dark Velvet Brown', bodyShape: 'Pear' },
        { skinTone: 'Olive Neutral', undertone: 'Neutral', hairColor: 'Warm Mahogany', bodyShape: 'Inverted Triangle' },
      ];
      const randomScan = results[Math.floor(Math.random() * results.length)];
      setScanResult(randomScan);
      setProfile((prev) => ({
        ...prev,
        skinTone: randomScan.skinTone,
        undertone: randomScan.undertone,
        hairColor: randomScan.hairColor,
        bodyShape: randomScan.bodyShape,
      }));
      setIsScanning(false);
    }, 1800);
  };
  ...
  const handleFinishOnboarding = () => {
    ...
    onSaveProfile({ ...profile, completedOnboarding: true });
    onClose();
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-fadeIn">
      <div className="bg-[#0f111a] border border-white/15 rounded-3xl max-w-2xl w-full p-6 sm:p-8 shadow-2xl shadow-amber-500/10 relative overflow-hidden text-slate-100">
        ...
        {step === 1 && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 items-center">
              <div className="relative group rounded-2xl overflow-hidden border border-white/15 bg-slate-900/60 aspect-square flex flex-col items-center justify-center p-4">
                <img
                ...
                {scanResult && (
                  <div className="p-4 rounded-2xl bg-amber-400/10 border border-amber-400/30 text-xs space-y-2 animate-fadeIn">
                    <div className="flex justify-between">
                      ...
                      <span className="font-bold text-amber-300">{scanResult.skinTone}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-400">Undertone:</span>
                      <span className="font-bold text-rose-300">{scanResult.undertone} Palette</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-400">Hair Tone:</span>
                      <span className="font-bold text-slate-200">{scanResult.hairColor}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-400">Body Silhouette:</span>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-slate-900 border border-white/10 rounded-xl px-3 py-2.5 text-sm text-white focus:border-amber-400 outline-none"
                />
              </div>

              <div>
                <label className="block text-xs font-semibold text-slate-400 mb-1">Chest / Bust (cm)</label>
                <input
                  type="number"
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-400 mb-1">Hips (cm)</label>
                <input
                  type="number"
            ...
            <div className="p-4 rounded-2xl bg-slate-900/80 border border-white/10 flex items-center gap-3">
              ...
              <div className="text-xs text-slate-300">
                Calculated Body Shape Profile: <strong className="text-amber-300">{profile.bodyShape}</strong>. 
                ...
                return (
                  <button
                    ...
                    className={`p-3.5 rounded-2xl border text-left text-xs font-bold transition-all flex items-center justify-between ${
                      selected
                        ? 'bg-gradient-to-r from-amber-500/20 to-rose-500/20 border-amber-400 text-amber-200 shadow-md shadow-amber-500/10'
        ...
        {step === 5 && (
          <div className="space-y-6 text-center py-4">
            ...
            <div>
              <h3 className="text-2xl font-serif font-bold text-white">Profile Configured!</h3>
            ...
            <div className="bg-slate-900/80 border border-white/10 rounded-2xl p-4 text-xs text-slate-300 text-left max-w-md mx-auto space-y-2">
              <div className="flex justify-between">
                <span className="text-slate-400">Skin Undertone:</span>
                <span className="text-amber-300 font-semibold">{profile.skinTone} ({profile.undertone})</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-400">Body Silhouette:</span>
                <span className="text-amber-300 font-semibold">{profile.bodyShape}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-400">Primary Occasions:</span>
                <span className="text-slate-200 font-semibold">{profile.selectedOccasions.join(', ') || 'All'}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-400">Style Vibes:</span>
                <span className="text-slate-200 font-semibold">{profile.styleVibes.join(', ') || 'Minimalist'}</span>
              </div>
            </div>
          </div>
        )}
