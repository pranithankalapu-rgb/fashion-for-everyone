...
export const ColorVotingView: React.FC = () => {
  ...
  const handleVote = (id: string, starRating: number) => {
    confetti({
      particleCount: 50,
      spread: 60,
      origin: { y: 0.7 },
    ...
    setCombos((prev) =>
      prev.map((c) => {
        if (c.id === id) {
          const newVotesCount = c.votesCount + 1;
          const newAvgRating = Number(
            ((c.rating * c.votesCount + starRating) / newVotesCount).toFixed(2)
          );
          return {
            ...c,
            rating: newAvgRating,
            ...
            userVote: starRating,
            trendingScore: c.trendingScore + 2,
          };
        }
        return c;
      })
  ...
  return (
    <div className="space-y-8 animate-fadeIn">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex items-center gap-2 bg-slate-900/80 p-1.5 rounded-2xl border border-white/10 overflow-x-auto max-w-full">
        ...
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-slate-900/80 border border-white/10 text-[11px] text-slate-300">
          ...
          <div key={combo.id} className="glass-card rounded-3xl p-6 flex flex-col justify-between space-y-6">
            ...
            <div>
              <div className="flex justify-between items-start">
                <span className="text-[10px] uppercase font-bold tracking-widest text-slate-400 bg-white/5 px-2.5 py-1 rounded-full border border-white/10">
              ...
              <h3 className="font-serif font-bold text-xl text-white mt-3">{combo.title}</h3>
            </div>

            {/* Color Palette Display */}
            <div className="space-y-2">
              <div className="h-16 rounded-2xl overflow-hidden flex shadow-inner border border-white/10">
            ...
            <div className="bg-slate-950/60 p-4 rounded-2xl border border-white/5 space-y-3">
              <div className="flex justify-between items-center text-xs">
                <div className="flex items-center gap-1">
                  ...
                  <strong className="text-white text-sm">{combo.rating}</strong>
                  <span className="text-slate-400">/ 5.0</span>
                </div>
                <span className="text-slate-400">{combo.votesCount.toLocaleString()} votes</span>
              </div>

              {/* Interactive 5-Star Vote Picker */}
              <div className="pt-2 border-t border-white/10 flex items-center justify-between">
                <span className="text-[11px] text-slate-400">Cast Your Vote:</span>
                <div className="flex gap-1">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <button
                      key={star}
                      onClick={() => handleVote(combo.id, star)}
                      className="p-1 text-slate-500 hover:text-amber-400 hover:scale-125 transition-all"
      ...
      {isSubmitOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
          <div className="bg-[#0f111a] border border-white/15 rounded-3xl max-w-md w-full p-6 space-y-6 shadow-2xl">
            <h2 className="text-xl font-serif font-bold text-white">Propose New Color Combination</h2>

            <form onSubmit={handleCreateCombo} className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-slate-400 mb-1">Combination Title</label>
                <input
                  type="text"
                  placeholder="e.g., Midnight Plum & Champagne Gold"
                  value={newTitle}
                  onChange={(e) => setNewTitle(e.target.value)}
                  className="w-full bg-slate-900 border border-white/10 rounded-xl px-3 py-2 text-sm text-white focus:border-rose-400 outline-none"
                  required
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-semibold text-slate-400 mb-1">Occasion</label>
                  <select
                    value={newOccasion}
                    onChange={(e) => setNewOccasion(e.target.value as any)}
                    className="w-full bg-slate-900 border border-white/10 rounded-xl px-3 py-2 text-xs text-white focus:border-rose-400 outline-none"
                  >
              ...
              <div className="space-y-2">
                <label className="block text-xs font-semibold text-slate-400">Color Palette Swatches</label>
                <div className="grid grid-cols-3 gap-2">
                  <input
                    type="color"
                    value={hex1}
                    onChange={(e) => setHex1(e.target.value)}
                    className="w-full h-10 rounded-xl border border-white/10 cursor-pointer bg-slate-900"
                  />
                  <input
                    type="color"
                    value={hex2}
